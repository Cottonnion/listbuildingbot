...
var prompt_enabled = siteConfig.enable_prompt;
var chatListeners = {};
var session_html = ''; //'<p class="lbb-referrer">This message was sent from url: '+current_url+'</p>';
/*var is_new_session = Cookies.get("lbb_chat_session");
var is_popup_display = Cookies.get("lbb_popup_display");
...
if(is_popup_display === undefined) {
	is_popup_display = true;
} else {
	is_popup_display = false;
}*/

function lbbValidateToken(default_chat_token = '') {
	if (default_chat_token != ''){
		chat_token = default_chat_token;
	}else{
...
if (siteConfig.is_wp_admin && firebaseConfig.length !== 0) {
	lbb_firebase_notifincation_init();
}

if (typeof chat_mode !== 'undefined' && chat_mode == 'live') {
...
/*if(current_admin_id !== "" && current_admin_id !== undefined ) {
	...
	users.on('value', function (users) {
		...
		if(data.key === current_admin_id) {
               if(user.isAdmin !== undefined && user.isAdmin) {
				notification_admin_id = data.key;
	...
	users.on("child_changed", (snapshot, prevChildKey) => {
		var changed_user_id = snapshot.key;
		if(changed_user_id === notification_admin_id) {
		var changed_data = snapshot.val();
		device_token = changed_data.tokens;
		}
	});
		...
		url: siteConfig.ajaxurl,
			...
			success: function(response) {
				...
				if(response_data.available == 1){
					...
					lbbCustomReponse(admin_has_joined,'lbb-special-messages');
					store_messsage_in_db(admin_has_joined, conversation_cookie_val, 1, 1);
					...
					jQuery('.connecting-agent').remove();
					lbbCustomReponse('Agent is not available at momment','lbb-special-messages');
				}
			}
		});
}
function get_current_user_data(fname = '', email = '',phone = '') {
	...
	var conversation_chat_cookie_key = 'lbbcf_'+lbb_chatflow_id+'_conversation_chat_id';
	var conversation_chat_cookie_val = '';
	if (getCookie(conversation_chat_cookie_key)){
		conversation_chat_cookie_val = getCookie(conversation_chat_cookie_key);
	}
	...
	var loadingtext = jQuery('#lbb_please_wait').val();
	jQuery('.lbb-chat-user-info-button').html(loadingtext);
			...
			success: function(response) {
				...
				if( response_data.key !== "" ) {
					...
					.then((userCredential) => {
						...
						var chat_db = firebase.database().ref('chat_data/' + current_role_id + '/message');
...
function load_user_message_for_admin(firebase_dynamic_id = ''){
	...
	if (firebase_dynamic_id == ''){
		var friend_id = jQuery('#message-to-send-user-id').val();
	}else{
		var friend_id = firebase_dynamic_id;
	}

	if (chatListeners[friend_id]) {
	...
	// Store the listener in the chatListeners object
  	chatListeners[friend_id] = chat_db;
}

function load_new_admin_child(snapshot) {
	chat_key_data = snapshot.key;

	if (jQuery('#lbb-chat-msg-id-'+chat_key_data).length){
		return;
	}
	...
	if(admin_user !== parseInt(chat_val_data.user_id)) {
		...
		latest_chat_message = `<li class="user-visitor-list chat-top-msg"  id="lbb-chat-msg-id-`+chat_key_data+`">
		...
		//Below event only call when you are viewving same user chat that time this notification will run
		if (typeof siteConfig.notify_notification !== 'undefined' && siteConfig.notify_notification && checkuser){
	      showNotification(msg_html, sender_id);
        }
        if (typeof siteConfig.notify_play_audio !== 'undefined' && siteConfig.notify_play_audio && checkuser){
			lbb_notification_ding();
        }
		...
		latest_chat_message = `<li class="user-admin-list"  id="lbb-chat-msg-id-`+chat_key_data+`">
	      <div class="message-box-img">
	        <span class="chat-icon"><img src="`+admin_avtar+`"></span>
	      </div>
	          <div class="message-box-data">
	            <div class="message-data">
	              <span class="message-data-name"> `+admin_name+`</span>
	              <span class="message-data-time">`+formattedTime+`</span>
	            </div>
	            <div class="message my-message">
	              <div class="lbb-user-message">${msg_html}</div>
	    ...
	    var checkuser = 1;
	    jQuery('.active-chat.user-admin-list-left-side .status').text(msg_html);
...
function load_new_child(snapshot) {
	...
	if(admin_user !== parseInt(chat_val_data.user_id)) {
		...
		lbbHandleUserResponse(latest_chat_message, false);
...
function sendMessageFromAdmin(current_role_id, message_text) {
	...
	var current_admin_id = jQuery("#current_admin_id").val();
	//var admin_firebase_id = jQuery("#current_admin_id").val();
    ...
    var message = firebase.database().ref('chat_data/' + current_role_id + '/message').push()
    message.set(msgData);
	
	firebase.database().ref('fcmTokens').child(current_role_id).once('value').then(function (data) {
		
    });
	
	attach_thread(current_role_id,current_admin_id);
...
function SendMessage(sendder_type = 'U') {
	var status = true;
	...
	if (sendder_type === 'A'){
		...
		var friend_id = jQuery('#message-to-send-user-id').val();
		var firebase_id = jQuery("#admin-firebase-id").val();
	    var admin_send = 1;
	...
	if (friend_id == "" || firebase_id == "" || text_message == "" || lbb_chatflow_id == ""){
		return;
	}

	/*if(is_new_session) {
		is_new_session = false;
		Cookies.set('lbb_chat_session',"yes");
...
function store_messsage_in_db(text_message, lbb_chatflow_id, admin_send, is_conversation_id = 0){
...
function attach_thread(current_role_id,current_admin_id) {
	
	var friendList = { friendId: current_role_id, userId: current_admin_id};
	...
	db.once('value', function (friends) {
		friends.forEach(function (data) {
			...
			if ((user.friendId === friendList.friendId && user.userId === friendList.userId) || ((user.friendId === friendList.userId && user.userId === friendList.friendId))) {
                flag = true;
                current_role_id = data.key;
            }
		});
	if(false === flag) {
		current_role_id = firebase.database().ref('friend_list').push(friendList, function (error) {
...
function lbb_init_firebase(){
	jQuery('#lbb-chat-main-wrapper').removeClass('lbb-chat-user-input');
	var current_role_id = jQuery("#current_role_id").val();
	var newItems = false;
	var chat_db = firebase.database().ref('chat_data/' + current_role_id + '/message');
	chat_db.on('child_added', (snapshot, prevChild) => {
		if (!newItems) return;
